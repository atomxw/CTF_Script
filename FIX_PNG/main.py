...
def icon():
    console.print(r"""
   ___                 ___  ___ _        _____          ___  _  _______
  / _ )__ ____ __ ___ |_  |/ _ ( )___   / __(_)_ ______/ _ \/ |/ / ___/
 / _  / // /\ \ /(_-</ __// // //(_-<  / _// /\ \ /___/ ___/    / (_ / 
/____/\_, //_\_\/___/____/\___/ /___/ /_/ /_//_\_\   /_/  /_/|_/\___/  
     /___/                                                             
""")
...
if __name__ == '__main__':
    ...
    fileName = args.f.split("\\")[-1]
    fileDir = os.path.abspath(args.f)
    ...
    if os.path.splitext(fileName)[1] != ".png":
        console.print("[-] 您的文件后缀名不为PNG!")
        time.sleep(1)
        exit(-1)

    icon()
    if crc32key == original_crc32: # 计算crc对比原始crc
        console.print("[-] 计算CRC32, 宽高没有问题, 开始尝试暴力破解!(感谢老铜匠)")
        BruteForceCrack.pngbaoli_def(args.f)
    else:
        executablePath = os.path.join(baseDir, "BrutePNG", "BrutePNG.exe")
        result = subprocess.run([f"{executablePath}", f"{fileDir}"], capture_output=True)
